...
import { fetchAmazonDeals } from "@/lib/source-amazon";

export async function POST() {
  try {
    let sourceUsed = "amazon";
    let freshDeals = await fetchAmazonDeals();
    ...
    const response: RefreshResponse = {
      ok: true,
      message: error instanceof Error ? `Amazon falló, usado fallback dummy: ${error.message}` : "Amazon falló, usado fallback dummy",
      count: deals.length,
      historyCount: history.length,
      sourceUsed: "dummy",
      triggeredAt: new Date().toISOString()
