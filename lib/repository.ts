...
export async function countDealsInDb(): Promise<number> {
  const result = await pool.query("SELECT COUNT(*)::int AS count FROM deals");
...
async function insertHistoryRow(dealId: number, price: number, pricePerPack: number) {
  await pool.query(
    `INSERT INTO price_history (deal_id, price, price_per_pack) VALUES ($1, $2, $3)`,
    [dealId, price, pricePerPack]
  );
}

async function upsertDeal(input: InputDeal): Promise<"inserted" | "updated" | "unchanged"> {
  ...
  if (existing.rowCount && existing.rows[0]) {
    ...
    await pool.query(
      `
      UPDATE deals
      SET source = $2, product_type = $3, set_name = $4, price = $5, price_per_pack = $6,
          score = $7, seller = $8, status = $9, url = $10, updated_at = NOW()
      WHERE id = $1
      `,
      [row.id, input.source, input.productType, input.setName, input.price, input.pricePerPack, input.score, input.seller, input.status, input.url]
    );
  ...
  const inserted = await pool.query(
    ...
    RETURNING id
    `,
    [input.title, input.source, input.productType, input.setName, input.price, input.pricePerPack, input.score, input.seller, input.status, input.url]
  );
...
export async function syncDeals(inputs: InputDeal[]) {
  await ensureSchema();
  let newDealsCount = 0;
  let changedDealsCount = 0;

  for (const input of inputs) {
    const result = await upsertDeal(input);
    if (result === "inserted") newDealsCount += 1;
    if (result === "updated") changedDealsCount += 1;
  }

  return { newDealsCount, changedDealsCount };
}

export async function seedIfEmpty() {
  await ensureSchema();
  const count = await countDealsInDb();
  if (count === 0) {
