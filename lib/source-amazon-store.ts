...
function inferSetName(title: string) {
  const knownSets = ["151", "Temporal Forces", "Surging Sparks", "Paldea Evolved", "Paradox Rift", "Twilight Masquerade"];
...
function parseStoreLinks(html: string, baseUrl: string): AmazonScrapeItem[] {
  ...
  while ((match = anchorRegex.exec(html)) !== null) {
    ...
    const price = parsePriceFromText(context) ?? parsePriceFromText(text);
    items.push({ title: text, url: absoluteUrl(href, baseUrl), price });
  }
...
function filterStoreItems(items: AmazonScrapeItem[]) {
  ...
  for (const item of filtered) {
    const key = [normalizeText(item.title), classifyProduct(item.title), inferSetName(item.title)].join("|");
...
export async function fetchAmazonOfficialStoreDeals(): Promise<Omit<Deal, "id" | "createdAt" | "updatedAt">[]> {
  ...
  if (!storeUrl) throw new Error("AMAZON_STORE_URL no está configurada");
  ...
  if (!response.ok) throw new Error(`Amazon store respondió con status ${response.status}`);
